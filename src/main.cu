...
struct ImageToTextureData{
    ...
    ImageToTextureData() { memset(this, 0, sizeof(*this)); }
...
bool LoadTextureFromFile(const char* filename, ImageToTextureData* tex_data){
    ...
    // create descriptor set using ImGui implementation
    ...
    // release image memory using stbi
...
void RemoveTexture(ImageToTextureData* tex_data)
{
...

    {
        for (int j = 0; j < ny; j++) {
            for (int i = 0; i < nx; i++) {
                ...
                auto ir = int(255.999 * fb[pixelIdx + 0]);
                auto ig = int(255.999 * fb[pixelIdx + 1]);
                auto ib = int(255.999 * fb[pixelIdx + 2]);
...
int main(){
    ...
    GLFWwindow* window = glfwCreateWindow(1920, 1080, "Ray Tracing in a Weekend with CUDA and ImGui and Vulkan", nullptr, nullptr);
